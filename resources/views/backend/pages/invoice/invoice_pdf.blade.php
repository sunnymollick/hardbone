...
<head>
    ...
    <style type="text/css">
        ...
        /* Footer - Use thead/tfoot approach */
        .footer-content {
            position: fixed;
            bottom: 0;
            left: 0;
...
<body>
    ...
    <div class="main-content">
        <table class="top-table" width="100%">
            <tr>
                <td align="left">
        ...
        <br>
        <h3 style="text-align:center;font-weight:bold;">Invoice Title : {{ $inv_data->title }}</h3>
        
        <table class="details" cellpadding='5'>
            <tr>
                <td width='35%' bordercolor='#ccc' bgcolor='yellowgreen'
                    style='font-size:15px; border:1px solid #001a00;'><strong>Description</strong></td>
                <td bgcolor='yellowgreen' style='font-size:15px; border:1px solid #001a00;'><strong>Qty</strong></td>
                <td bgcolor='yellowgreen' style='font-size:15px; border:1px solid #001a00;'><strong>Unit</strong></td>
                <td bgcolor='yellowgreen' style='font-size:15px; border:1px solid #001a00;'><strong>Unit Price ({{ $currency }})</strong></td>
            ...
            @foreach ($groupedDetails as $category => $categoryDetails)
                ...
                @foreach ($categoryDetails as $info)
                    <tr>
                        ...
                        <td valign='top' style='font-size:14px; border: 1px solid #001a00; text-align:right; padding-right:5px'>
                            {{ $info->total_price }}</td>
                    </tr>
                @endforeach
            @endforeach
            ...
            @if ($inv_data->grand_total)
                <tr>
                    ...
                    <td style='font-size:14px; color:tomato; text-align:right; padding-right:5px'>{{ $currency }} {{ $subTotal }}</td>
                </tr>
                <tr>
                    <td colspan="3"></td>
                    <td style='font-size:14px;'>Discount (-)</td>
                    <td style='font-size:14px; color:tomato; text-align:right; padding-right:5px'>{{ $currency }} {{ $discount_amount }}</td>
                </tr>
                <tr>
                    <td colspan="3"></td>
                    <td style='font-size:14px;'>Tax (+)</td>
                    <td style='font-size:14px; color:tomato; text-align:right; padding-right:5px'>{{ $currency }} {{ $tax }}</td>
                </tr>
                <tr>
                    <td colspan="3"></td>
                    <td style='font-size:14px;'>Grand Total</td>
                    <td style='font-size:14px; color:tomato; text-align:right; padding-right:5px'>{{ $currency }} {{ $grand_total }}</td>
                </tr>
                <tr>
                    <td colspan="3"></td>
                    <td style='font-size:14px;'>Paid Amount</td>
                    <td style='font-size:14px; color:tomato; border: 1px solid #001a00; text-align:right; padding-right:5px '>
        ...
        @if ($inv_data->bank_details)
            ...
            <table width='100%' cellspacing='0' cellpadding='10' border='1' bordercolor='#CCCCCC'>
                <tr>
                    <td width='100%' bordercolor='#ccc' bgcolor='yellowgreen'
                        style='font-size:14px;'><strong>Bank Details</strong></td>
                </tr>
                <tr>
    ...
    <div class="footer-content">
        <table class="footer-table" cellspacing='0' cellpadding='5'>
            <tr>
                <td width='33%' style='font-size:16px; color:red' valign='top'>
                    <b>{{ $company_details->app_name }}</b>
                </td>
                <td width='34%' style='font-size:13px;' align='center' valign='top'>
                    <strong>{{ $company_details->address }}<br />
                        @if ($company_details->address_secondary)
                            Second Address: {{ $company_details->address_secondary }}<br />
                        @endif
                        Phone: {{ $company_details->phone_1 }}
                    </strong>
                </td>
                <td valign='top' width='33%' style='font-size:12px;' align='right'>
                    <!-- Page number can go here if needed -->
                </td>
            </tr>
        </table>
    </div>

    <!-- Script to add footer to each page -->
    <script type="text/php">
        if (isset($pdf)) {
            $text = "Page {PAGE_NUM} of {PAGE_COUNT}";
            $size = 10;
            $font = $fontMetrics->getFont("Tahoma");
